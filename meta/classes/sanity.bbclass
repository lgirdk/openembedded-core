...
def check_tar_version(sanity_data):
    ...
    if LooseVersion(version) < LooseVersion("1.28"):
        bb.warn("Your version of tar (%s) is older than 1.28 and does not have the support needed to enable reproducible builds. Please install a newer version of tar (you could use the project's buildtools-tarball from our last release or use scripts/install-buildtools)." % version)
    return None
